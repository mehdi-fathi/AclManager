...
class AcosTable extends Table
{
    ...
    {
        $validator//** add unique for validation role **//         
            ->add('alias',['unique' =>['rule' =>'validateUnique', 
            'provider' => 'table', 
            'message' => 'Is Not unique']]
            );
    ...
    {      
       $validator
            ->add('alias', ['uniquechildren' =>['rule' =>'uniquechildren', 
            'provider' => 'table', 
            'message' => 'Not unique']]
            );
    ...
    public function isUnique($value, array $context)
    {
       ...
       // debug($value);
        ...
        if(is_array($value)){//** if we have command :create  **//
            
             $node = $this->findByAlias($value[0])->where(['id'=>$context['data']['parent_id']])->find('all');
